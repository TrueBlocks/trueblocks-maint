...
import "github.com/TrueBlocks/trueblocks-maint/v2/internal/state"
...
func (a *App) GetTableState(n string) state.TableState {
	return a.state.GetTableState(n)
}

func (a *App) SetTableState(n string, t state.TableState) {
	a.state.SetTableState(n, t)
}
...
func (a *App) SetTab(pageName, tab string) {
	a.state.SetTab(pageName, tab)
}

func (a *App) SaveWindowGeometry(x, y, w, h int) {
	a.state.SetWindowGeometry(x, y, w, h)
}
